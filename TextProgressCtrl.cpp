...
void CTextProgressCtrl::OnPaint()
{
	...
	CMemDCAlt dc(&PaintDC);						// memory device context
	...
	{
		double percent;
		if ((m_nMax-m_nMin)>=10000)
		{
			percent=((dFraction * 100.0) + 0.004);
			if (percent>100.0) percent=100.0;
			str.AppendFormat(_T("%0.2f%%"), percent);
		}
		else
		if ((m_nMax-m_nMin)>=1000)
		{
			percent=((dFraction * 100.0) + 0.04);
			if (percent>100.0) percent=100.0;
			str.AppendFormat(_T("%0.1f%%"), percent);
		}
		else
		{
			percent=((dFraction * 100.0) + 0.5);
			if (percent>100.0) percent=100.0;
			str.AppendFormat(_T("%d%%"), (int)(percent));
		}
	}
	if (str.GetLength())
	{
