...
BOOL CZoomscrolldemoDlg::OnInitDialog()
{
	...
	if (!Chart::setLicenseCode("RDST-25PS-KMGP-AU23-5C83-6407",NULL))
	{
		MessageBox("Chart Director license key failure","Error",MB_OK|MB_ICONASTERISK);
	}
	...
	else
	{
		...
		if (m_timeStampsA.len) m_minDate = m_timeStampsA[0]; 
			...
			if (m_timeStampsC.len) m_minDate = m_timeStampsC[0];
		else 
			if (m_timeStampsD.len) m_minDate = m_timeStampsD[0];
		else 
			if (m_timeStampsE.len) m_minDate = m_timeStampsE[0];
		else 
			if (m_timeStampsF.len) m_minDate = m_timeStampsF[0];
		else 
			if (m_timeStampsG.len) m_minDate = m_timeStampsG[0];
		else
			m_minDate=0;
		...
		if (m_timeStampsD.len) if (m_timeStampsD[0]<m_minDate) m_minDate=m_timeStampsD[0];
		if (m_timeStampsE.len) if (m_timeStampsE[0]<m_minDate) m_minDate=m_timeStampsE[0];
		if (m_timeStampsF.len) if (m_timeStampsF[0]<m_minDate) m_minDate=m_timeStampsF[0];
		if (m_timeStampsG.len) if (m_timeStampsG[0]<m_minDate) m_minDate=m_timeStampsG[0];
		...
		if (m_timeStampsA.len) m_maxDate = m_timeStampsA[m_timeStampsA.len-1];
			...
			if (m_timeStampsC.len) m_maxDate = m_timeStampsC[m_timeStampsC.len-1];
		else
			if (m_timeStampsD.len) m_maxDate = m_timeStampsD[m_timeStampsD.len-1];
		else
			if (m_timeStampsE.len) m_maxDate = m_timeStampsE[m_timeStampsE.len-1];
		else
			if (m_timeStampsF.len) m_maxDate = m_timeStampsF[m_timeStampsF.len-1];
		else
			if (m_timeStampsG.len) m_maxDate = m_timeStampsG[m_timeStampsG.len-1];
		else
			m_maxDate=1;
		...
		if (m_timeStampsD.len) if (m_timeStampsD[m_timeStampsD.len-1]>m_maxDate) m_maxDate=m_timeStampsD[m_timeStampsD.len-1];
		if (m_timeStampsE.len) if (m_timeStampsE[m_timeStampsE.len-1]>m_maxDate) m_maxDate=m_timeStampsE[m_timeStampsE.len-1];
		if (m_timeStampsF.len) if (m_timeStampsF[m_timeStampsF.len-1]>m_maxDate) m_maxDate=m_timeStampsF[m_timeStampsF.len-1];
		if (m_timeStampsG.len) if (m_timeStampsG[m_timeStampsG.len-1]>m_maxDate) m_maxDate=m_timeStampsG[m_timeStampsG.len-1];
...
void CZoomscrolldemoDlg::drawChart(CChartViewer *viewer)
{
	...
	// Get the starting index of the array using the start date
	int startIndexD = (int)(std::lower_bound(m_timeStampsD.data, m_timeStampsD.data + m_timeStampsD.len,viewPortStartDate) - m_timeStampsD.data);
	if ((startIndexD > 0) && (m_timeStampsD[startIndexD] != viewPortStartDate)) 
		--startIndexD;
	// Get the ending index of the array using the end date
	int endIndexD = (int)(std::upper_bound(m_timeStampsD.data, m_timeStampsD.data + m_timeStampsD.len,viewPortEndDate) - m_timeStampsD.data);
	if (endIndexD >= m_timeStampsD.len - 1)
		endIndexD = m_timeStampsD.len - 1;

	// Get the starting index of the array using the start date
	int startIndexE = (int)(std::lower_bound(m_timeStampsE.data, m_timeStampsE.data + m_timeStampsE.len,viewPortStartDate) - m_timeStampsE.data);
	if ((startIndexE > 0) && (m_timeStampsE[startIndexE] != viewPortStartDate)) 
		--startIndexE;
	// Get the ending index of the array using the end date
	int endIndexE = (int)(std::upper_bound(m_timeStampsE.data, m_timeStampsE.data + m_timeStampsE.len,viewPortEndDate) - m_timeStampsE.data);
	if (endIndexE >= m_timeStampsE.len - 1)
		endIndexE = m_timeStampsE.len - 1;

	// Get the starting index of the array using the start date
	int startIndexF = (int)(std::lower_bound(m_timeStampsF.data, m_timeStampsF.data + m_timeStampsF.len,viewPortStartDate) - m_timeStampsF.data);
	if ((startIndexF > 0) && (m_timeStampsF[startIndexF] != viewPortStartDate)) 
		--startIndexF;
	// Get the ending index of the array using the end date
	int endIndexF = (int)(std::upper_bound(m_timeStampsF.data, m_timeStampsF.data + m_timeStampsF.len,viewPortEndDate) - m_timeStampsF.data);
	if (endIndexF >= m_timeStampsF.len - 1)
		endIndexF = m_timeStampsF.len - 1;

	// Get the starting index of the array using the start date
	int startIndexG = (int)(std::lower_bound(m_timeStampsG.data, m_timeStampsG.data + m_timeStampsG.len,viewPortStartDate) - m_timeStampsG.data);
	if ((startIndexG > 0) && (m_timeStampsG[startIndexG] != viewPortStartDate)) 
		--startIndexG;
	// Get the ending index of the array using the end date
	int endIndexG = (int)(std::upper_bound(m_timeStampsG.data, m_timeStampsG.data + m_timeStampsG.len,viewPortEndDate) - m_timeStampsG.data);
	if (endIndexG >= m_timeStampsG.len - 1)
		endIndexG = m_timeStampsG.len - 1;

	// Get the length
	int noOfPointsA = endIndexA - startIndexA + 1;
	int noOfPointsB = endIndexB - startIndexB + 1;
	int noOfPointsC = endIndexC - startIndexC + 1;
	int noOfPointsD = endIndexD - startIndexD + 1;
	int noOfPointsE = endIndexE - startIndexE + 1;
	int noOfPointsF = endIndexF - startIndexF + 1;
	int noOfPointsG = endIndexG - startIndexG + 1;
	...
	double* viewPortTimeStampsD = new double[noOfPointsD];
	double* viewPortTimeStampsE = new double[noOfPointsE];
	double* viewPortTimeStampsF = new double[noOfPointsF];
	double* viewPortTimeStampsG = new double[noOfPointsG];
	...
	double* viewPortDataSeriesD = new double[noOfPointsD];
	double* viewPortDataSeriesE = new double[noOfPointsE];
	double* viewPortDataSeriesF = new double[noOfPointsF];
	double* viewPortDataSeriesG = new double[noOfPointsG];
	double* viewPortPointTypesA = new double[noOfPointsA];
	...
	int arraySizeInBytesD = noOfPointsD * sizeof(double);
	int arraySizeInBytesE = noOfPointsE * sizeof(double);
	int arraySizeInBytesF = noOfPointsF * sizeof(double);
	int arraySizeInBytesG = noOfPointsG * sizeof(double);
	...
	if (m_pointTypes) memcpy(viewPortPointTypesA, m_pointTypesA.data + startIndexA, arraySizeInBytesA);
	if (m_multiline)
	{
		...
		memcpy(viewPortTimeStampsD, m_timeStampsD.data + startIndexD, arraySizeInBytesD);
		memcpy(viewPortDataSeriesD, m_dataSeriesD.data + startIndexD, arraySizeInBytesD);
		memcpy(viewPortTimeStampsE, m_timeStampsE.data + startIndexE, arraySizeInBytesE);
		memcpy(viewPortDataSeriesE, m_dataSeriesE.data + startIndexE, arraySizeInBytesE);
		memcpy(viewPortTimeStampsF, m_timeStampsF.data + startIndexF, arraySizeInBytesF);
		memcpy(viewPortDataSeriesF, m_dataSeriesF.data + startIndexF, arraySizeInBytesF);
		memcpy(viewPortTimeStampsG, m_timeStampsG.data + startIndexG, arraySizeInBytesG);
		memcpy(viewPortDataSeriesG, m_dataSeriesG.data + startIndexG, arraySizeInBytesG);
	}
#if 0
	if ((noOfPointsA >= 520))
	{
	...
	if (m_multiline&&(noOfPointsD >=520))
	{
			ArrayMath mD(DoubleArray(viewPortTimeStampsD, noOfPointsD));
			mD.selectRegularSpacing(noOfPointsD / 260);
			int aggregatedNoOfPointsD = mD.aggregate(DoubleArray(viewPortTimeStampsD, noOfPointsD),Chart::AggregateFirst).len;
			mD.aggregate(DoubleArray(viewPortDataSeriesD, noOfPointsD), Chart::AggregateAvg);
			//mD.aggregate(DoubleArray(viewPortDataSeriesD, noOfPointsD), Chart::AggregateMed);
			noOfPointsD = aggregatedNoOfPointsD;
	}
	if (m_multiline&&(noOfPointsE >=520))
	{
			ArrayMath mE(DoubleArray(viewPortTimeStampsE, noOfPointsE));
			mE.selectRegularSpacing(noOfPointsE / 260);
			int aggregatedNoOfPointsE = mE.aggregate(DoubleArray(viewPortTimeStampsE, noOfPointsE),Chart::AggregateFirst).len;
			mE.aggregate(DoubleArray(viewPortDataSeriesE, noOfPointsE), Chart::AggregateAvg);
			//mE.aggregate(DoubleArray(viewPortDataSeriesE, noOfPointsE), Chart::AggregateMed);
			noOfPointsE = aggregatedNoOfPointsE;
	}
#endif
	//
    ...
    // Add the first data series
		if (m_timeStampsA.len)
		{
			...
			if (m_pointTypes)
			{
				c->addScatterLayer(DoubleArray(viewPortTimeStampsA, noOfPointsA),
        ArrayMath(DoubleArray(viewPortDataSeriesA, noOfPointsA) ).selectEQZ(ArrayMath(DoubleArray(viewPortPointTypesA, noOfPointsA)).sub(0), Chart::NoValue), 
				"Disconnected", 
				Chart::Cross2Shape(), 
				4, 
				0x0000FF,
				...
				c->addScatterLayer(DoubleArray(viewPortTimeStampsA, noOfPointsA),
        ArrayMath(DoubleArray(viewPortDataSeriesA, noOfPointsA) ).selectEQZ(ArrayMath(DoubleArray(viewPortPointTypesA, noOfPointsA)).sub(1), Chart::NoValue), 
				"Connected", 
				Chart::DiamondShape, 
				4, 
				0xFF0000,
			...
			LineLayer *layerOn = c->addLineLayer();
			if (!m_pointTypes)
			...
			{
				//layer0->addDataSet(DoubleArray(viewPortDataSeriesA, noOfPointsA), 0xff0000, m_dataTitleA);
				(layer0->addDataSet(ArrayMath(DoubleArray(viewPortDataSeriesA, noOfPointsA) ).selectEQZ(ArrayMath(DoubleArray(viewPortPointTypesA, noOfPointsA)).sub(2), Chart::NoValue), 0xff0000, m_dataTitleA+" Off"))->setDataSymbol(Chart::DiamondShape, 3,-1,-1,1);
				layer0->setGapColor(0xff0000,-1);

				(layerOn->addDataSet(ArrayMath(DoubleArray(viewPortDataSeriesA, noOfPointsA) ).selectEQZ(ArrayMath(DoubleArray(viewPortPointTypesA, noOfPointsA)).sub(3), Chart::NoValue), 0x00ff00, m_dataTitleA+" On"))->setDataSymbol(Chart::TriangleShape, 3,-1,-1,1);
				layerOn->setXData(DoubleArray(viewPortTimeStampsA, noOfPointsA));
				layerOn->setLineWidth(1);
				layerOn->setGapColor(0x00ff00,-1);
				layerOn->setFastLineMode(1);
			}
			layer0->setXData(DoubleArray(viewPortTimeStampsA, noOfPointsA));
			layer0->setLineWidth(1);
			layer0->setFastLineMode(1);
		}
		if (m_multiline)
		{
			...
			if (m_timeStampsC.len)
			{
				...
				layer2->setFastLineMode(1);
			}
			if (m_timeStampsD.len)
			{
				// Add the third data series
				LineLayer *layer3 = c->addLineLayer();
				(layer3->addDataSet(DoubleArray(viewPortDataSeriesD, noOfPointsD), 0x00ffff, m_dataTitleD))->setDataSymbol(Chart::InvertedTriangleShape, 3,-1,-1,1);
				layer3->setXData(DoubleArray(viewPortTimeStampsD, noOfPointsD));
				layer3->setLineWidth(1);
				layer3->setFastLineMode(1);
			}
			if (m_timeStampsE.len)
			{
				// Add the third data series
				LineLayer *layer4 = c->addLineLayer();
				(layer4->addDataSet(DoubleArray(viewPortDataSeriesE, noOfPointsE), 0xffff00, m_dataTitleE))->setDataSymbol(Chart::RightTriangleShape, 3,-1,-1,1);
				layer4->setXData(DoubleArray(viewPortTimeStampsE, noOfPointsE));
				layer4->setLineWidth(1);
				layer4->setFastLineMode(1);
			}
			if (m_timeStampsF.len)
			{
				// Add the data series
				LineLayer *layer5 = c->addLineLayer();
				(layer5->addDataSet(DoubleArray(viewPortDataSeriesF, noOfPointsF), 0xff00ff, m_dataTitleF))->setDataSymbol(Chart::LeftTriangleShape, 3,-1,-1,1);
				layer5->setXData(DoubleArray(viewPortTimeStampsF, noOfPointsF));
				layer5->setLineWidth(1);
				layer5->setFastLineMode(1);
			}
			if (m_timeStampsG.len)
			{
				// Add the data series
				LineLayer *layer6 = c->addLineLayer();
				(layer6->addDataSet(DoubleArray(viewPortDataSeriesG, noOfPointsG), 0xff80ff, m_dataTitleG))->setDataSymbol(Chart::SquareShape, 3,-1,-1,1);
				layer6->setXData(DoubleArray(viewPortTimeStampsG, noOfPointsG));
				layer6->setLineWidth(1);
				layer6->setFastLineMode(1);
			}
		}
  ...
  m_bmp = c->makeChart(Chart::BMP);
	...
	delete[] viewPortTimeStampsD;
	delete[] viewPortTimeStampsE;
	delete[] viewPortTimeStampsF;
	delete[] viewPortTimeStampsG;
	delete[] viewPortDataSeriesA;
	delete[] viewPortDataSeriesB;
	delete[] viewPortDataSeriesC;
	delete[] viewPortDataSeriesD;
	delete[] viewPortDataSeriesE;
	delete[] viewPortDataSeriesF;
	delete[] viewPortDataSeriesG;
	delete[] viewPortPointTypesA;
}
...
void CZoomscrolldemoDlg::OnBnClickedButtonSavechart()
{
	...
	CFileDialog fileDialog(FALSE,".jpg",fileNameDefault,OFN_OVERWRITEPROMPT, "JPG|*.jpg|JPEG|*.jpeg|PNG|*.png|GIF|*.gif|WBMP|*.wbmp|BMP|*.bmp|SVG|*.svg|SVGZ|*.svgz|PDF|*.pdf||",NULL,0,TRUE);
